...
The Genesia node runs before skills are discovered and rewrites the user request into an "enhanced prompt." By the time the skill is loaded, the specialist receives conflicting instructions — Genesia's rewritten spec AND the skill's directives. The intent classifier adds unnecessary complexity since the router already handles task classification.

## Decision

Surgically remove the Genesia prompt enhancement node and intent classifier. The pipeline becomes skill-driven: every task is executed via a skill (discovered or ephemeral).
...
- `genesia_build_specification` — prompt enhancement node
- `genesia_build_sub_specification` — sub-task prompt enhancement
- `GENESIA_SYSTEM_PROMPT` — the "expert prompt engineer" system prompt
...
The specialist currently receives `specification` (Genesia's enhanced output). After refactor:
...
- **Sub-tasks**: router seed spec + skill content + sibling context (no Genesia expansion)
...
| File | Change |
|------|--------|
| `agents/graph.py` | Remove intent_classifier + genesia nodes/edges. Wire START → Router directly. Remove spec critic loop. Flatten complexity tiering. |
| `agents/nodes.py` | Remove `classify_task`, `genesia_build_specification`, `genesia_build_sub_specification`, `_parse_genesia_output` |
| `agents/specialist_nodes.py` | Update `execute_with_specialist` to use `user_request` instead of `specification` as primary input |
| `agents/adapters.py` | Remove `GENESIA_SYSTEM_PROMPT` |
| `agents/state.py` | Remove spec-critic fields, remove `get_context_for_node("genesia")` path |
| `agents/decision_functions.py` | Remove `should_approve_specification`, `should_generate_code` |

## Test Impact

Tests that directly test the Genesia node or spec critic loop will need updating. Tests for router, skill loader, specialist, critic, tool calling, and security should pass unchanged.
