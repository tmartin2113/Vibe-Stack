...
def _execute_checked_out_task(
    ...
    if final_state.get("upgrade_applied"):
        upgrade_desc = final_state.get("upgrade_proposal_description", "")
        upgrade_branch = final_state.get("upgrade_branch", "")
        upgrade_commit = final_state.get("upgrade_commit", "")

        # Create a dedicated Paperclip issue for the upgrade proposal
        # so it appears in the sidebar and can be filtered by label.
        try:
            upgrade_issue = client.create_issue(
                title=f"Self-Upgrade: {upgrade_desc[:80]}",
                description=_format_upgrade_issue_body(
                    description=upgrade_desc,
                    branch=upgrade_branch,
                    commit=upgrade_commit,
                    source_issue_id=issue.id,
                ),
                priority="low",
                labels=["self-upgrade", "auto-generated"],
            )
            logger.info(
                "Created self-upgrade issue %s for branch %s",
                upgrade_issue.id, upgrade_branch,
            )
            metrics.increment("vibe_paperclip_api_calls_total", labels={"endpoint": "create_issue"})
        except PaperclipAPIError as e:
            logger.warning("Failed to create self-upgrade issue (non-fatal): %s", e)

        # Also comment on the original issue for cross-reference
