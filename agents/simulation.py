...
from .simulation_adapters import register_simulation_adapters
from .simulation_budget import (
    SimulationBudget,
    assess_simulation_budget as _assess_simulation_budget_impl,
    _probe_free_vram,
    _nvidia_smi_free_mb,
    _MIN_FREE_VRAM_MB_SIDECAR,
    _MAX_PERSONA_ROUNDS,
    _CLARIFICATION_CONFIDENCE_THRESHOLD,
    _VET_SKILLS_ENABLED,
)

logger = logging.getLogger(__name__)

# Re-evaluate kill-switch at module level so that test reloads of this
# module pick up env var changes (simulation_budget is not reloaded in
# those tests, so its module-level _SIM_ENABLED may be stale).
...
def assess_simulation_budget(
    ...
    Thin wrapper around simulation_budget.assess_simulation_budget that
    applies the locally-evaluated _SIM_ENABLED kill-switch.  This allows
    test reloads of this module to pick up env var overrides without also
    requiring simulation_budget to be reloaded.
    """
    if not _SIM_ENABLED:
        return SimulationBudget(
            enabled=False, reason="Disabled via VIBE_SIM_ENABLED=false"
        )
    return _assess_simulation_budget_impl(system_profile=system_profile, mode=mode)
